    ...
    render() {
        ...
        return (
            <Card >
                    ...
                    <Card.Footer content={`${timeago(product.pubTime)}发布 ${timeafter(product.expireTime)}过期`} extra={<div>{`发布人：${product.user}`}</div>} />
                    </Card>
        );
    }
...
function timeafter(dateTimeStamp){   //dateTimeStamp是一个时间毫秒，注意时间戳是秒的形式，在这个毫秒的基础上除以1000，就是十位数的时间戳。13位数的都是时间毫秒。
    var result
    var minute = 1000 * 60;      //把分，时，天，周，半个月，一个月用毫秒表示
    var hour = minute * 60;
    var day = hour * 24;
